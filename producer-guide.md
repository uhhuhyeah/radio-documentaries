...
7. With all script parts recorded, publishing to Navidrome is **split into two workflows** (Navidrome's music mount is read-only to the agent — see `## Navidrome`):
  ...
  - **7b. Prompted publish.** *After* the rescan, when David prompts it, the agent completes all publish artifacts before marking the row `published`: `publish_episode` creates the detailed source/cue playlist and records its ID/URL, `compile_episode_track` creates the single long-form episode track and records its compiled song ID, and `publish_compiled_season_playlist` updates the season playlist of compiled episode tracks. Only after all three succeed does it set the `seasons.md` status to **`published`** and fill the Published date.
  - The end result: David can load the detailed cue playlist for the full interleaved episode, or the compiled season playlist where each episode is a single long-form track.
