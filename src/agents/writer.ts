...
export async function writeScript(input: WriterInput): Promise<string> {
  const user = [
    ...
    `Emit BARE --- front matter (never a \`\`\`yaml fence), KEBAB-CASE slot labels, and NO markdown`,
    `in spoken bodies. Write the FULL ${input.targetMinutes ?? 25}-minute script now — be thorough and`,
    `expansive across many SPOKEN parts (aim ~4,000 spoken words total); do not stop short.`,
    ``,
